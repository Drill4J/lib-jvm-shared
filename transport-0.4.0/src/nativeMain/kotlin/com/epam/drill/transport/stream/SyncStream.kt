...
import com.epam.drill.transport.lang.alloc2
import com.epam.drill.transport.lang.invalidOp
import com.epam.drill.transport.lang.smallBytesPool
import com.epam.drill.transport.lang.unsupported
...
open class SyncStreamBase : Closeable,
    SyncRAInputStream,
    SyncRAOutputStream,
    SyncLengthStream {
...
interface Extra {
    ...
    class Mixin(override var extra: LinkedHashMap<String, Any?>? = null) :
        Extra
...
fun MemorySyncStream(data: ByteArrayBuilder) = MemorySyncStreamBase(
    data
).toSyncStream()
...
class MemorySyncStreamBase(var data: ByteArrayBuilder) : SyncStreamBase() {
    ...
    fun checkPosition(position: Long) = run { if (position < 0) invalidOp(
