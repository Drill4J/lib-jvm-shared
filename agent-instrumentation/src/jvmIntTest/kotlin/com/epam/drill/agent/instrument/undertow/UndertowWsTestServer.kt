...
object UndertowWsTestServer {

    fun withWebSocketAnnotatedEndpoint(clazz: Class<*>, block: (String) -> Unit) = WebSocketDeploymentInfo()
        .addEndpoint(clazz)
        .let { withWebSocketEndpoint(it, block) }

    fun withWebSocketInterfaceEndpoint(clazz: Class<out Endpoint>, block: (String) -> Unit) = WebSocketDeploymentInfo()
        .addEndpoint(ServerEndpointConfig.Builder.create(clazz, "/").build())
        .let { withWebSocketEndpoint(it, block) }
