...
internal object HpackUtil {


    /**
     * A string compare that doesn't leak timing information.
     */
    fun equals(s1: ByteArray?, s2: ByteArray?): Boolean {
        if (s1!!.size != s2!!.size) {
            return false
        }
        var c = 0.toChar()
        for (i in s1.indices) {
            c = (c.code or (s1[i].toInt() xor s2[i].toInt())).toChar()
        }
        return c.code == 0
    }
    ...
    val HUFFMAN_CODE_LENGTHS = byteArrayOf(
