...
import com.epam.drill.agent.ws.topic.*
...
@SharedImmutable
private val logger = KotlinLogging.logger("com.epam.drill.agent.ws.WsSocket")
...
class WsSocket : CoroutineScope {
    ...
    fun connect(adminUrl: String) {
        ...
        headers = {
            mutableMapOf(
                HEADER_AGENT_CONFIG to ProtoBuf.encodeToHexString(AgentConfig.serializer(), agentConfig),
                HEADER_CONTENT_ENCODING to "deflate"
            )
        }
        ...
        wsClient.onBinaryMessage { rawMessage ->
            val message = ProtoBuf.decodeFromByteArray(Message.serializer(), rawMessage)
