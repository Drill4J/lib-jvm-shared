...
class PropertiesFileProviderTest {
    ...
    fun `config path from installation dir`() {
        ...
        assertEquals("/data/agent/drill.properties", result)
    }
    ...
    fun `config path from property`() {
        val result = PropertiesFileProvider(setOf(SimpleMapProvider(mapOf(
            DefaultParameterDefinitions.INSTALLATION_DIR.name to "/data/agent",
            DefaultParameterDefinitions.CONFIG_PATH.name to "/data/agent/config.properties"
        )))).configPath()
        assertEquals("/data/agent/config.properties", result)
    }
    ...
    fun `from installation dir one entry`() {
        val provider1 = SimpleMapProvider(mapOf("foo" to "bar1"))
        val provider2 = SimpleMapProvider(mapOf(DefaultParameterDefinitions.INSTALLATION_DIR.name to "/data1"))
        val result = PropertiesFileProvider(setOf(provider1, provider2)).fromInstallationDir()
        assertEquals("/data1/drill.properties", result)
    }

    @Test
    fun `from installation dir two entry prioritized`() {
        val provider1 = SimpleMapProvider(mapOf(DefaultParameterDefinitions.INSTALLATION_DIR.name to "/data1"), 100)
        val provider2 = SimpleMapProvider(mapOf(DefaultParameterDefinitions.INSTALLATION_DIR.name to "/data2"), 200)
        val result = PropertiesFileProvider(setOf(provider1, provider2)).fromInstallationDir()
        assertEquals("/data2/drill.properties", result)
    }
